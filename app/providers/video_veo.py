"""Veo 3.1 image-to-video provider — high-quality + audio alternative (CLAUDE.md §4.7, §5).

연결: Gemini API / Vertex / fal / Replicate. veo-3.1 (Lite/Fast/Quality), $0.03–0.40/s.
여기서는 fal 경유(FAL_KEY)로 호출; 키 없으면 ffmpeg Ken Burns 오프라인 베이스라인.
"""
...
class VeoProvider(VideoProvider):
    name = "veo-3.1"
    endpoint = "fal-ai/veo/3.1/image-to-video"
    min_duration, max_duration = 4.0, 8.0   # Veo 3.1: ~4–8s
            ...
            with httpx.Client(timeout=600) as client:
                r = client.post(
                    ...
                    json={"prompt": prompt, "image_url": b64,
                          "duration": self.clamp_duration(duration), "generate_audio": with_audio},
                )
                r.raise_for_status()
